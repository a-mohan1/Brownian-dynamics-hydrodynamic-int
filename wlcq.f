...
c     (1+4/3*delt*Hspr)*Q**3 - (2+3*delt*Hspr+R)*Q**2 + (1+2*delt*Hspr+2*R)*Q - R == 0
c     lying in (0, 1)
c     Inputs:
...
c     delt = time step
      ...
      real*8 function wlcq(R, Hspr, delt)
      implicit none
      integer maxit, iter
      real*8 R, Hspr, delt
...
c     c3, c2, c1, c0 = coeffs in cubic eq
      c3 = 1.0 + 4.0/3.0*delt*Hspr
      c2 = -(2.0 + 3.0*delt*Hspr + R)
      c1 = (1.0 + 2.0*delt*Hspr + 2.0*R)
      c0 = -R
...
c     Reaching here means we have failed to converge
      write(*, *) 'wlcq failed; max iterations exceeded'
      write(*, *) 'bye'
      stop
