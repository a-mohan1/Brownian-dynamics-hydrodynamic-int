...
c     of Hsieh et al. and Schroeder et al.
...
c     whicHI = which HI tensor to use (FD = 0, OB = 1, RPY = 2, Blake = 3)
c     a = bead hydrodynamic radius 
...
c     nseed = seed for RNG (negative integer if whicRNG = 1)
c     delt = time step
c     Nks = number of Kuhn lengths per spring
c     v = EV parameter, used only if isgev is .TRUE.
c     kappa = 3-by-3 transposed vel gradient tensor (non-dimensionalized with 
...
c     Dful = diffusion tensor
c     Bful = 'square root' of diffusion tensor
c     update = .TRUE. if Dful and Bful must be updated at istep
...
c     In Dful and Bful, the diffusion tensor relating the force at bead n to the 
c     velocity at bead m is stored as a 3-by-3 block for every pair m and n,
c     and m runs over rows, while n runs over columns 

      subroutine pc1s(xc, yc, zc, xn, yn, zn, nbeads, istep, whicHI, a,
     &                sprtype, Hspr, whicRNG, ranflag, nseed, delt, Nks,
     &                v, kappa, tether, Dful, Bful, update, overext, 
     &                isgev, isflow, istab)

      implicit none
      integer nbeads, istep, whicHI, sprtype, whicRNG, ranflag, nseed
      integer i, j, icoord, jcoord, ibead, icount, jcount
      real*8 xc(nbeads), yc(nbeads), zc(nbeads)
      real*8 xn(nbeads), yn(nbeads), zn(nbeads)
      real*8 a, Hspr, delt, Nks, v, kappa(3, 3), rannum
      real*8 Dful(3*nbeads, 3*nbeads), Bful(3*nbeads, 3*nbeads)
      ...
      logical tether, update, overext, isgev, isflow, istab
      
c     i = spring index, runs from 1..nbeads-1
c     ibead, j = bead indices, run from 1..nbeads
c     icoord, jcoord = coord indices, run from 1..3
c     icount, jcount = index from 1..3*nbeads
c     (or 1..3*(nbeads-1), if used over springs)

c     For uniformity's sake, will use i for outer spring loops,
c     ibead for outer bead loops, j for inner bead loops, 
c     icoord and icount for outer loops,
c     and jcoord and jcount for inner loops.
...
c     overext is initialized to .FALSE. in calcFspr and
c     also unnecessarily (but without consequence) in Fspr1b,
c     and need not be initialized here
c     Subroutine Fspr1b is not used here
...
c     compute the diffusion tensor Dful and its lower triangular decomposition 
c     Bful at istep if update is .TRUE.
c     otherwise, the tensors supplied by the calling routine will be used
      if(update) then
         call Dfull(xc, yc, zc, nbeads, istep, whicHI, a, Dful)
         call cholesky(Dful, 3*nbeads, istep, Bful)
      endif
c     These subroutines will overwrite existing Dful, Bful matrices
...
c       Now calculate the terms that must be summed over beads j = 1..nbeads
        do 90 j = 1, nbeads
         do 100 jcoord = 1, 3
          jcount = 3*(j-1)+jcoord

c         Add the spring forces
c         Must treat j=1 (first bead) and j=nbeads (last bead) separately
c         For j=1, spring force is only from spring 1     
c         (first 3 elements of fspful)
c         For j=nbeads, spring force is only from spring nbeads-1
c         (last 3 elements of fspful) and takes a negative sign
c         For bead j, take the diff between forces from springs j and j-1

          if(j.eq.1) then
            rnew(icount) = rnew(icount) + 
     &      Dful(icount, jcount)*fspful(jcount)*delt
          elseif(j.eq.nbeads) then                
            rnew(icount) = rnew(icount) + 
     &      Dful(icount, jcount)*(-fspful(jcount-3))*delt
          else
            rnew(icount) = rnew(icount) + 
     &      Dful(icount, jcount)*(fspful(jcount)-fspful(jcount-3))*delt
          endif 

c         Now add the EV force terms, if necessary
          if(isgev) then
           rnew(icount) = rnew(icount) + 
     &     Dful(icount, jcount)*fevful(jcount)*delt
          endif

c         Now add the Brownian terms
          rnew(icount) = rnew(icount) + 
     &    const*Bful(icount, jcount)*n(jcount)
...
c        Add the EV and Brownian terms
         do 130 jcount = 1, 3*nbeads   
           tevb(i, icoord) = tevb(i, icoord) + 
     &     (Dful(3*i+icoord, jcount) - Dful(3*(i-1)+icoord, jcount))* 
     &     fevful(jcount)*delt +
     &     const * ( Bful(3*i+icoord, jcount) - 
     &     Bful(3*(i-1)+icoord, jcount) )* n(jcount)  
 130     continue
 120    continue
 110  continue
...
c      First, calculate Qprime for i from the x, y, z values at istep+1
...
c       Now, add the term assoc with the force from spring i in dir icoord
        icount = 3*(i-1)+icoord
        rhsc1(icoord) = rhsc1(icoord) + 2.0*delt*fspful(icount)

c       Now add the spring force terms that must be summed over beads j
c       For j<i, Qc1 vector is known, and must be used to update fspful
c       For j>=i, must take the spring forces from fspful at time istep
...
c       fspful is calculated using Qc1 for springs < i, and Qc for i and beyond
     ...
     &      fspful(jcount)*delt
           elseif(j.eq.nbeads) then                
            rhsc1(icoord) = rhsc1(icoord) + 
     &      (Dful(3*i+icoord, jcount) - Dful(3*(i-1)+icoord, jcount))*  
     &      (-fspful(jcount-3))*delt
           else
            rhsc1(icoord) = rhsc1(icoord) + 
     &      (Dful(3*i+icoord, jcount) - Dful(3*(i-1)+icoord, jcount))*  
     &      (fspful(jcount)-fspful(jcount-3))*delt
           endif 
 320      continue
 300    continue        
...
c       Now, add the term assoc with the force from spring i in dir icoord
c       Recall that fspful now contains spring forces calculated from Qc1 vectors
c       for springs >= i and from Qf for springs < i

        icount = 3*(i-1)+icoord
        rhsc2(icoord) = rhsc2(icoord) + 2.0*delt*fspful(icount)
 
c       Now add the spring force terms that must be summed over beads j
c       For j<i, Qf vector is known, and must be used to update fspful
c       For j>=i, must take the spring forces from fspful computed using Qc1
